...
// Get available divisions (not already in holdout list)
$availableDivisions = LocationManagement::getAvailableDivisionsForHoldout($locationId);

header_html('Add Division Holdout');
?>

<h2>Add Division Holdout: <?= h($location['name']) ?></h2>
...
<?php if (empty($availableDivisions)): ?>
  <div class="card">
    <p>All divisions have already been added to the holdout list for this location.</p>
    <a class="button" href="/locations/edit.php?id=<?= (int)$locationId ?>">Back to Location</a>
  </div>
<?php else: ?>
  <div class="card">
    <form method="post" action="/locations/holdout_add_eval.php" class="stack">
      ...
      <div class="actions">
        <button class="primary" type="submit">Add Division Holdout</button>
